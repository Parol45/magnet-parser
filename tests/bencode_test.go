...
func TestConversion(t *testing.T) {
	...
	// ------------------------------------------------------------------------------------------------------
	bcode, err := bencode_converters.JSONToBencode(firstInpJson)
	if err != nil {
		t.Fatal(err)
	}
	...
	// ------------------------------------------------------------------------------------------------------
	json, err := bencode_converters.BencodeToJSON([]byte(firstBcode))
	if err != nil {
		t.Fatal(err)
	}
	if json != firstOutJson {
		t.Fatal(fmt.Sprintf("Failed at first test: %s != %s", json, firstOutJson))
	}
	// ------------------------------------------------------------------------------------------------------
	// ------------------------------------------------------------------------------------------------------
	...
	secondJson := `["iche",-42]`
	// ------------------------------------------------------------------------------------------------------
	bcode, err = bencode_converters.JSONToBencode(secondJson)
	if err != nil {
		t.Fatal(err)
	}
	...
	// ------------------------------------------------------------------------------------------------------
	json, err = bencode_converters.BencodeToJSON([]byte(secondBcode))
	if err != nil {
		t.Fatal(err)
	}
	if json != secondJson {
		t.Fatal(fmt.Sprintf("Failed at second test: %s != %s", json, secondJson))
	}
	// ------------------------------------------------------------------------------------------------------
	// ------------------------------------------------------------------------------------------------------
	...
	// ------------------------------------------------------------------------------------------------------
	bcode, err = bencode_converters.JSONToBencode(complexJsonInp)
	if err != nil {
		t.Fatal(err)
	}
	...
	// ------------------------------------------------------------------------------------------------------
	json, err = bencode_converters.BencodeToJSON([]byte(complexBcode))
	if err != nil {
		t.Fatal(err)
	}
	if json != complexJsonOut {
		t.Fatal(fmt.Sprintf("Failed at complex test: %s != %s", json, complexJsonOut))
	}
	// ------------------------------------------------------------------------------------------------------
	// ------------------------------------------------------------------------------------------------------
